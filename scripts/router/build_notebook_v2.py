"""
Build slm_router_qwen3_colab_a100_v2.ipynb programmatically.

This avoids the notorious notebook-JSON '\n' pitfall by constructing
cells as Python strings, then serializing properly into the .ipynb format.

Usage:
    python scripts/router/build_notebook_v2.py
"""

import json
from pathlib import Path

OUTPUT_PATH = Path(__file__).parent / "slm_router_qwen3_colab_a100_v2.ipynb"


def make_cell(source: str, cell_type: str = "code") -> dict:
    """Create a notebook cell dict from a Python string."""
    return {
        "cell_type": cell_type,
        "metadata": {},
...
def fix_source(lines: list[str]) -> list[str]:
    """Ensure each line (except last) ends with exactly one newline."""
    if not lines:
        return lines
    fixed = []
...
# ── Cell 5: System Prompt (15 intents — from config.py) ──
# Use triple-quote via variable assignment to avoid \n issues
CELLS.append(make_cell(
    "SYSTEM_PROMPT = (\n"
...
# Use raw triple-quoted string to avoid quoting nightmares
_CELL18 = (
    'NL_inf = chr(10)\n'
    'test_cases = [\n'
...
# ── Cell 19: GGUF Export ──
CELLS.append(make_cell(
    '# Export GGUF Q4_K_M for laptop deployment (~2.6GB)\n'
    'print("Exporting GGUF Q4_K_M (for laptop)...")\n'
    'model.save_pretrained_gguf(\n'
    '    str(DRIVE_DIR / "gguf" / "q4_k_m"),\n'
    '    tokenizer,\n'
    '    quantization_method="q4_k_m",\n'
    ')\n'
    'print("Q4_K_M done!")\n'
    '\n'
    '# Also export Q8_0 for maximum accuracy\n'
    'print("Exporting GGUF Q8_0 (maximum accuracy)...")\n'
    'model.save_pretrained_gguf(\n'
    '    str(DRIVE_DIR / "gguf" / "q8_0"),\n'
    '    tokenizer,\n'
    ...
    '\n'
    'import os\n'
    'for qdir in ["q4_k_m", "q8_0"]:\n'
    '    p = DRIVE_DIR / "gguf" / qdir\n'
    '    if p.exists():\n'
    '        for f in p.iterdir():\n'
...
# ── Cell 21: Push to HuggingFace Hub (optional) ──
CELLS.append(make_cell(
...
def build_notebook():
    # Fix source arrays: ensure each line except last has trailing newline
    for cell in CELLS:
    ...
    # Validate: parse every code cell with ast
    import ast
    errors = 0
    for i, cell in enumerate(CELLS):
        ...
        # Skip cells with %%capture or ! shell commands
    ...
    if errors == 0:
        ...
        print(f"  ❌ {errors} cells have syntax errors")
